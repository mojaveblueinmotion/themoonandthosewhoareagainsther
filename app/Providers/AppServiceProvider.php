...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        //     \Mail::alwaysTo('always-to@email.com');
        // }
        $shouldBeStrict = env('MODEL_STRICT', false);
    ...
    public function newBaseMacro()
    {
        Blueprint::macro(
            'commonFields',
            function ($fields = 'created_by|updated_by|created_at|updated_at') {
                if (is_string($fields)) {
                ...
                foreach ($fields as $field) {
                    switch (trim($field)) {
                        case 'created_by':
                            ...
                            $this->foreign('created_by')->references('id')->on('sys_users');
                            break;
                        case 'updated_by':
                            $this->unsignedBigInteger('updated_by')->nullable();
                            $this->foreign('updated_by')->references('id')->on('sys_users');
                            break;
                        case 'created_at':
        ...
        Collection::macro(
            'paginate',
            function ($perPage = 10, $total = null, $page = null, $pageName = 'page') {
        ...
        Route::macro(
            'grid',
            function ($uri, $controller, $params = []) {
                    ...
                    foreach ($params['with'] as $action) {
                        switch ($action) {
                            ...
                            case 'approval':
                                ...
                                break;
                            case 'excel':
                                Route::get("{$uri}/{record}/excel", "{$controller}@excel")->name("{$uri}.excel");
                                break;
                            case 'print':
        ...
        Builder::macro(
            'whereLike',
            function ($attributes, string $keyword) {
                $this->where(
                    function (Builder $query) use ($attributes, $keyword) {
                        foreach (\Arr::wrap($attributes) as $attribute) {
                            $query->when(
                                ...
                                function (Builder $query) use ($attribute, $keyword) {
                                    ...
                                    $query->orWhereHas(
                                        $relationPath,
                                        function (Builder $query) use ($attributeField, $keyword) {
