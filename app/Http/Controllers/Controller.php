...
class Controller extends BaseController
{
    ...
    public function render($view, $additional = [])
    {
        if (auth()->user()->status === 'nonactive') {
            return view('dashboard.nonactive');
        }
    ...
    public function checkGeneralColumn($params)
    {
        if ($params === 'name:num') {
            ...
            $params = 'name:created_by|label:Dibuat Oleh|sortable:false|width:180px';
        } elseif ($params === 'name:created_at') {
            $params = 'name:created_at|label:Dibuat Pada|sortable:false|width:180px';
        } elseif ($params === 'name:updated_by') {
            $params = 'name:updated_by|label:#|sortable:false|width:40px';
        } elseif ($params === 'name:updated_at') {
            $params = 'name:updated_at|label:#|width:130px';
        ...
        if (isset($params['type'])) {
            switch ($params['type']) {
                ...
                case 'show':
                    ...
                    $settings['url']     = $params['url'] ?? rut($this->data['routes'] . '.show', $params['id']);
                    break;
                ...
                case 'edit':
                    ...
                    $settings['url']     = $params['url'] ?? rut($this->data['routes'] . '.edit', $params['id']);
                    break;
                ...
                case 'delete':
                    ...
                    $settings['url']     = $params['url'] ?? rut($this->data['routes'] . '.destroy', $params['id']);
                ...
                case 'activate':
                    ...
                    $settings['url']   = $params['url'] ?? rut($this->data['routes'] . '.activate', $params['id']);
                ...
                if (!empty($item['type'])) {
                    ...
                    switch ($item['type']) {
                        ...
                        case 'show':
                            ...
                            $default['label'] = 'Lihat';
                            $default['url'] = rut($this->prepared('routes') . '.show', $item['id']);
                            $default['attrs'] = 'data-modal-backdrop="true"';
                            break;
                        case 'approval':
                            $default['icon'] = 'fa fa-check text-primary';
                            $default['label'] = 'Approval';
                            $default['url'] = $item['url'] ?? rut($this->prepared('routes') . '.approval', $item['id']);
                            break;
                        case 'edit':
                            $default['icon'] = $item['icon'] ?? 'fa fa-edit text-success';
                            $default['label'] = $item['label'] ?? 'Ubah';
                            $default['url'] = $item['url'] ?? rut($this->prepared('routes') . '.edit', $item['id']);
                            break;
                        case 'print':
                            $default['icon'] = 'fa fa-print text-dark';
                            $default['label'] = 'Cetak';
                            $default['url'] = $item['url'] ?? rut($this->prepared('routes') . '.print', $item['id']);
                            $default['class'] = '';
                            $default['attrs'] = 'target="_blank"';
                            break;
                        case 'excel':
                            $default['icon'] = 'fa fa-file-excel text-success';
                            $default['label'] = 'Cetak';
                            $default['url'] = $item['url'] ?? rut($this->prepared('routes') . '.excel', $item['id']);
                            $default['class'] = '';
                            $default['attrs'] = 'target="_blank"';
                            break;
                        case 'download':
                            ...
                            $default['url'] = $item['url'] ?? rut($this->prepared('routes') . '.download', $item['id']);
                            $default['class'] = '';
                            $default['attrs'] = 'target="_blank"';
                            break;
                        case 'delete':
                            ...
                            $default['url'] = $item['url'] ?? rut($this->prepared('routes') . '.destroy', $item['id']);
                            if (!empty($item['text'])) {
                        ...
                        case 'history':
                            ...
                            $default['url'] = $item['url'] ?? rut($this->prepared('routes') . '.history', $item['id']);
                            $default['attrs'] = 'data-modal-backdrop="default"';
                            break;
                        case 'tracking':
                            $default['icon'] = 'fas fa-chart-line text-info';
                            $default['label'] = 'Tracking Approval';
                            $default['url'] = $item['url'] ?? rut($this->prepared('routes') . '.tracking', $item['id']);
                            $default['attrs'] = 'data-modal-backdrop="default"';
                            break;
                    }
                }
