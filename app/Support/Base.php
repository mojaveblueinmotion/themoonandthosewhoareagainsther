...
class Base
{
    ...
    public static function getModules($moduleName = false, $prefix = false)
    {
        $modules = [];
    ...
    public static function getModulesSecondary($moduleSearch, $moduleName = false)
    {
        $modules = [];
        foreach (config('backendmenu') as $menu) {
            if (!empty($menu['name']) && !empty($menu['title'])) {
                if($menu['name'] == $moduleSearch){
                    $name = $menu['name'];
                    $title = __($menu['title']);
                    // $modules[$name] = $title;
                    if (!empty($menu['submenu'])) {
                        foreach ($menu['submenu'] as $submenu) {
                            ...
                            if (!empty($submenu['submenu'])) {
                                foreach ($submenu['submenu'] as $submenu2) {
                                    $subname2 = $submenu2['name'];
                                    $subtitle2 = __($submenu2['title']);
                                    $modules[$subname2] = $subtitle . '/' . $subtitle2;
                                }
                            }
                        }
                    }
    ...
    public static function getPermsMenu($moduleName = false, $prefix = false)
    {
        $modules = [];
        foreach (config('backendmenu') as $menu) {
            if (!empty($menu['name']) && !empty($menu['title'])) {
                $name = $menu['name'];
                $title = __($menu['title']);
                $modules[$name] = $menu['perms'] ?? '';
                if (!empty($menu['submenu'])) {
                    foreach ($menu['submenu'] as $submenu) {
                        $subname = $submenu['name'];
                        $subtitle = __($submenu['title']);
                        $modules[$subname] = $submenu['perms'] ?? '';
                        if (!empty($submenu['submenu'])) {
                            foreach ($submenu['submenu'] as $submenu2) {
                                $subname2 = $submenu2['name'];
                                $subtitle2 = __($submenu2['title']);
                                $modules[$subname2] = $submenu2['perms'] ?? '';
                                if($prefix){
                                    $modules[$subname] = $submenu2['perms'] ?? '';
                                }
                            }
                        }
                    }
    ...
    public static function renderMenuTree($menu)
    {
        ...
        if (!empty($menu['section'])) {
            ...
            $first = '<li class="has-sub" data-title="' . __($menu['title']) . '">
                        <a href="' . ($menu['url'] === 'javascript:;' ? 'javascript:;' : yurl($menu['url'])) . '" class="' . ($menu['url'] !== 'javascript:;' ? 'base-content--replace' : '') . ' menu-link" data-name="' . $menu['name'] . '">';
            ...
            // $str = '<li class="' . $menuActive . '" data-title="' . __($menu['title']) . '"><a href="' . $menu['url'] . '" class="' . (empty($menu['reload']) ? 'base-content--replace' : '') . ' menu-link" data-name="' . $menu['name'] . '">';
            $str = '<li class="' . $menuActive . '" data-title="' . __($menu['title'])  . '"><a href="' . ($menu['url'] === 'javascript:;' ? 'javascript:;' : yurl($menu['url'])) . '" class="base-content--replace menu-link" data-name="' . $menu['name'] . '">';
            if (!empty($menu['icon'])) {
    ...
    public static function renderAsideMenu($menu)
    {
        ...
        if (!empty($menu['section'])) {
            ...
            $str = '<li class="menu-item menu-item-submenu" aria-haspopup="true" data-menu-toggle="hover" data-title="' . __($menu['title'])  . '">
                        <a href="' . ($menu['url'] === 'javascript:;' ? 'javascript:;' : yurl($menu['url'])) . '" class="menu-link menu-toggle">';
            if (!empty($menu['icon'])) {
                $str .= static::renderIcon($menu['icon']);
            ...
            $str = '<li class="menu-item ' . $menuActive . '" aria-haspopup="true" data-title="' . __($menu['title'])  . '">
                        <a href="' . ($menu['url'] === 'javascript:;' ? 'javascript:;' : yurl($menu['url'])) . '" class="menu-link base-content--replace" data-name="' . $menu['name'] . '">';
            if (!empty($menu['icon'])) {
                $str .= static::renderIcon($menu['icon']);
            } elseif (!empty($menu['bullet'])) {
                $str .= '<i class="menu-bullet menu-bullet-' . $menu['bullet'] . '">
