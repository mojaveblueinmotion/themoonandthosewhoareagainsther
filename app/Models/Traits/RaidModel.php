...
trait RaidModel
{
    ...
    {
        return '<div data-order="' . ($this->updated_at ?: $this->created_at) . '" class="text-center make-td-py-0">
            <small>
                <div class="text-nowrap">
                    <i data-toggle="tooltip" title="' . \Str::title($this->creatorName()) . '"
                        class="fa fa-user fa-fw fa-lg mr-2"></i>
                    <i data-toggle="tooltip" title="' . $this->created_at->format('d M Y, H:i:s') . '"
                        class="fa fa-clock fa-fw fa-lg"></i>
                </div>
            </small>
        </div>';
    }
    public function rescheduleByRaw()
    {
        return '<div data-order="' . ($this->reschedule_at) . '" class="text-left make-td-py-0">
                    <small>
                        <div class="creator-grid" style="display:flex;">
                            <div class="icon">
                        ...
                        <div class="text-nowrap">
                            <i data-toggle="tooltip" title="' . $this->rescheduleDate() . '"
    ...
    public function getCityRoot()
    {
        ...
        return $root->city ? $root->city->name : '';
    }
    ...
    function getTanggalFormatMulaiSelesai($tanggal_awal, $tanggal_akhir, $separation = " - ", $summary = false, $monthFull = false)
    {
        ...
        if ($tanggal_awal === $tanggal_akhir) {
            ...
            if ($monthFull === true) {
                return $tanggal_awal->translatedFormat('d M') . $separation . $tanggal_akhir->translatedFormat('d M') . " " . $tanggal_awal->translatedFormat('Y');
            } else {
                return $tanggal_awal->translatedFormat('d M') . $separation . $tanggal_akhir->translatedFormat('d M') . " " . $tanggal_awal->translatedFormat('Y');
            }
        } else {
