...
class InherentRisk extends Model
{
    use HasApprovals;

    protected $table = 'trans_inherent_risk';
    ...
    {
        return $query->filterBy(['inherent_risk_id', '='])
    ...
    public function handleSubmitSave($request)
    {
        ...
        try {
            $this->fill($request->all());
            $menu = \App\Models\Globals\Menu::where('module', 'risk-assessment.inherent-risk')->first();
        ...
        try {
            if ($this->status === 'waiting.approval.revisi') {
                ...
                if ($this->firstNewApproval($request->module . '_upgrade')) {
                    ...
                    $this->riskRegisterDetail->currentRisk->details()->delete();
                    $this->riskRegisterDetail->currentRisk->delete();
                }
            } else {
                ...
                if ($this->firstNewApproval($request->module)) {
                    ...
                    $currentRisk = CurrentRisk::firstOrNew(
                        [
                            'risk_register_detail_id' => $this->risk_register_detail_id,
    ...
    public function checkAction($action, $perms)
    {
        ...
        switch ($action) {
            ...
            case 'revisi':
                ...
                if (!empty($this->riskRegisterDetail->currentRisk)) {
                    if ($this->riskRegisterDetail->currentRisk->status == 'completed') {
                        $checkStatus = false;
                    }
                }
                return $checkStatus;
