    ...
    <div class="row">
        <div class="col-12 col-sm-6 col-md-3 mr-n6">
            <select class="form-control base-plugin--select2-ajax filter-control filter-type-laporan" data-post="type"
                ...
                @foreach (App\Http\Controllers\Report\ReportPelaksanaanAuditController::TYPE as $key => $value)
        ...
        <div class="col-12 col-sm-6 col-md-2 mr-n6 pb-2">
            <select name="type_id" data-post="type_id"
                ...
                data-url="{{ rut('ajax.selectTypeAudit', ['search' => 'all']) }}" {{ $menu == '' ? 'disabled' : '' }}
        ...
        $(function() {
            $('.content-page')
                .on('change', 'select.filter-control.filter-type-laporan', function(e) {
                    var me = $(this);
                    if (me.val()) {
                        BaseContent.replaceByUrl("{{ url('report/pelaksanaan-audit') }}/" + this.value);
                    }
