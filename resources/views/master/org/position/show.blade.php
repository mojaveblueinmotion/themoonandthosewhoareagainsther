...
@section('modal-body')
    <div class="form-group row">
        ...
        <div class="col-sm-12 parent-group">
            <input type="text" value="{{ $record->location->name ?? '' }}" class="form-control"
                placeholder="{{ __('Struktur') }}" disabled>
        </div>
    </div>
    <div class="form-group row">
        ...
        <div class="col-sm-12 parent-group">
            <select name="level_id" class="form-control base-plugin--select2-ajax"
                data-url="{{ rut('ajax.selectLevelPosition', ['search' => 'all']) }}"
                data-url-origin="{{ rut('ajax.selectLevelPosition', ['search' => 'all']) }}"
