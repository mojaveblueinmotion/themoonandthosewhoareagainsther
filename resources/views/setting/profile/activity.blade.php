...
@section('page')
    <div class="d-flex flex-row">
        @include($views . '.includes.profile-aside', ['tab' => 'activity'])
            ...
            <div class="card card-custom gutter-b">
                ...
                <div class="card-header py-3">
                    <div class="card-title align-items-start flex-column">
                        ...
                        <span class="text-muted font-weight-bold font-size-sm mt-1">{{ __('Riwayat Aktivitas') }}</span>
                    </div>
                </div>
                                        ...
                                        <div class="col-12 col-sm-6 col-xl-5 pb-2">
                                            <select id="moduleMain"
                                                ...
                                                @foreach (\Base::getModulesMain() as $key => $name)
                                                    @if (in_array($key, ['dashboard','monitoring-temuan' ,'monitoring', 'report']))
                                                        @continue
                                                    @endif
                                                    @if ($key == 'auth_login')
                                                        @continue
                                                    @elseif($key == 'auth_logout')
