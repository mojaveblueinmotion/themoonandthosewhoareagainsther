<script>
	$(function () {
        ...
        $('.content-page').on('keyup', '#harga, #jumlah, #biaya_bongkar_ampera, #premi_supir, #premi_agen, #bongkaran', function(e) {
            countPembayaran();
        });
		// handleExtPart();
	});
...
<script>
	var countPembayaran = function () {
		...
		var premi_supir = $('#premi_supir');
		var premi_agen = $('#premi_agen');
		var bongkaran = $('#bongkaran');
		var total_dibayar = $('#total_dibayar');
		var pengeluaran_lapak = $('#pengeluaran_lapak');
		...
		total_dibayar.val(parseInt(jumlah.val().replace(/,/g, '')) + parseInt(biaya_bongkar_ampera.val().replace(/,/g, ''))
			+ parseInt(premi_supir.val().replace(/,/g, ''))
			+ parseInt(premi_agen.val().replace(/,/g, ''))
			+ parseInt(bongkaran.val().replace(/,/g, '')) + parseInt(totalSum))
		pengeluaran_lapak.val(parseInt(jumlah.val().replace(/,/g, '')) + parseInt(biaya_bongkar_ampera.val().replace(/,/g, ''))
			+ parseInt(premi_supir.val().replace(/,/g, ''))
			+ parseInt(premi_agen.val().replace(/,/g, ''))
			+ parseInt(bongkaran.val().replace(/,/g, '')) + parseInt(totalSum))
	};
